...
void output_init(output_st *this, const uv_adc_channels_e adc_chn, const uv_gpios_e gate_io,
		...
		const uint16_t moving_avg_count, esb_emcy_e emcy_overload, esb_emcy_e emcy_fault) {
	...
	this->state = ESB_OUTPUT_STATE_OFF;
	this->current = 0;
}


/// @brief: Sets the output state
void output_set_state(output_st *this, const esb_output_state_e state) {
	if ((this->state != ESB_OUTPUT_STATE_FAULT) &&
			(this->state != ESB_OUTPUT_STATE_OVERLOAD)) {
		this->state = state;
	}
	else if (state == ESB_OUTPUT_STATE_OFF) {
		this->state = state;
	}
}
...
void output_step(output_st *this, uint16_t step_ms) {
	if (this->state == ESB_OUTPUT_STATE_ON) {
		if ((this->current > this->limit_max)) {
			uv_gpio_set(this->gate_io, false);
			output_set_state(this, ESB_OUTPUT_STATE_OVERLOAD);
		}
		else if (this->current < this->limit_min) {
			uv_gpio_set(this->gate_io, false);
			output_set_state(this, ESB_OUTPUT_STATE_FAULT);
		}
		else {
			uv_gpio_set(this->gate_io, true);
		}
	}
	else if (this->state == ESB_OUTPUT_STATE_OVERLOAD) {
		uv_gpio_set(this->gate_io, false);
		// send EMCY message
	...
	else if (this->state == ESB_OUTPUT_STATE_FAULT) {
		uv_gpio_set(this->gate_io, false);
		if (this->emcy_fault) {
